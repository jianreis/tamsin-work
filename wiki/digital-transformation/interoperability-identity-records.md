...
REST [[src-dig-fhir-medication-resources]]. It connects systems more readily
...
(lawful basis, purpose limitation, data-subject rights), so any unified patient
...
- **Design for an absent national backbone** today, but standards-align so you can
  plug into NHI/FHIR when it exists. 🔮 a national FHIR-based e-Rx exchange would
  change the picture across providers; flag as requires-reform.
