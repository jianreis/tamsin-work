...
expectation [[src-con-adjacent-digital-benchmarks]]. For price-sensitive shoppers
rewards are not a loyalty nicety but a **primary reason to choose a retailer**.
...
flow, and close the loop with ratings/feedback.
...
familiar pattern. A scripting experience that lives where people
...
- Run a WhatsApp-first conversational flow for submit/track/refill, meeting users where
  they already transact.
- Make payment transparent, multi-rail and instant, with USSD/cash parity for the low end.
- Show live status, an honest ETA and a named provider for both delivery and collection,
  and close the loop with ratings.
- Surface savings in-flow (generics, loyalty, co-pay) as a primary purchase
