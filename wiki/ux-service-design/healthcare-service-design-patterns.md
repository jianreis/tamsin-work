...
For a scripting service the dimensions that carry the most weight are **information &
...
In a health service, trust is built and lost in the details.
...
- **Privacy by context** - health status is sensitive, so default to discreet
...
Treat **every** user as possibly not fully understanding, applying *health-literacy
...
- Make **consent explicit and revocable, with no dark patterns**; default to
  **discreet** health notifications; minimise re-disclosure
  [[src-ux-sludge-thaler-sunstein]], anchored to [[wiki/sa-regulatory/_index|POPIA]].
