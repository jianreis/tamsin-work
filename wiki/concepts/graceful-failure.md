...
first-class experiences, on the premise that in SA pharmacy they are routine
rather than edge cases. The experience is judged not on the happy path but on
...
The common thread: an unexplained failure is the trust-killer, while an explained
failure with a clear next step and a visible human fallback can *build*
