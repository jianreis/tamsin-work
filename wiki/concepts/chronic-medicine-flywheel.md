...
who never runs out and never re-does admin has no reason to switch, and the pharmacy
...
and mirrored in the public sector by CCMDD. The scheme machinery (PMB/Chronic-Disease-List
registration, DSP nomination) is what authorises the
auto-dispense, so the flywheel must be wired into the funding rails rather than bolted on.
...
rails. Treat a lapsed registration or expired repeat as a P1 incident rather than a
