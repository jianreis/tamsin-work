...
insurance settlement**, all inside one app. It is also the clearest demonstration
...
loop and the guardrails; the platform scale and insurance plumbing don't transfer.
...
- **No first visit online** — online diagnosis/prescribing is limited to **follow-up
  ...
  diagnosis. This is *why* the platforms concentrate on **chronic refills**, not
...
- **O2O: route an online order to the nearest store for fast delivery** (AliHealth)
  — fits Dis-Chem/Clicks store estates.
...
- How much of China's loop depends on state direction that SA cannot replicate is
  hard to isolate — treat the *guardrails* as the safest transfer.
