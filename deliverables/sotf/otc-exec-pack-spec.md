---
type: deliverable-spec
title: "OTC solve - executive pack: structure, slide plan and draft copy"
project: sotf
status: draft
...
# OTC solve - executive pack spec

The spec for the pack that goes to the executives / Steer Group after the two
OTC workshops (2 Sep internal, 10 Sep frontline). The deliverable itself will
be `deliverables/sotf/otc-exec-pack.html` with a `.pptx` twin. This file is
written first and the HTML mirrors it, the same way the FoS journeys were
built. Prose voice: [[writing-style-guide]] plus the `tamsins-voice` skill.

## Structure (agreed with Tamsin, 2026-09-16)

Four sections, each named as the question it answers, with a one-slide answer
in front and an evidence appendix behind.
...
option scored against the original intents (never turn your back,
...
Status key: `drafted` (copy below, awaiting Tamsin's read) · `needs input`
...
House rule: titles are proposed, then agreed, before they lock.

| Slide | Proposed title |
|---|---|
...
1. Decoupled counters. The customer hands in at one counter and collects at
   another, with the picking done out of sight in between. In a legacy store
   the pharmacist takes the script, turns round, and hunts stock for up to
   five minutes while the customer stands at the counter. Decoupling keeps the
   pharmacist facing the customer and turns the picking into a production
   line, so a long script no longer holds up the counter and a customer with
   time to wait can roam the store instead of standing in a queue.
2. One queue for everything. Scripts and over-the-counter medicine (OTC:
   ...
   pharmacy, one set of staff to train, and no second counter to run. It
   also changes how customers think. With one counter for everything, they
   stop treating scripts and OTC as two separate errands and stop forgetting
   to ask for the OTC item at the dispensing counter. Every visit becomes one
   combined script-and-OTC conversation, and one combined sale.
...
customer, fulfil like a production line, keep the counter for care. The pack
scores every option against them.

Evidence: [[src-sotf-steerco1-concept-2025]], [[src-sotf-steerco-decisions-2025]],
[[src-bigly-blueprint-board-style]], [[src-sotf-otc-workshop-2026]] (space
rationale, legacy OTC wall objections).

### 1.2 Two stores are live. They run the pharmacy two different ways

Standfirst: Melrose Arch runs the pharmacy as it was trained. Irene runs the
...
- Roam until called to the drop-off counter. The dispenser takes the script or
  the OTC request, captures it, and upsells there.
- Roam again while the order goes through pick-and-pack. The dashboard
  prioritises the work.
- Called to the collections counter. Sign, take the bag.
- Pay at the front-shop till (payment at the pharmacy counter is planned for
  2028).

The same five steps apply whether the customer has a 24-item script or a
single box of antihistamine.

Right panel, Irene (live since July 2026, the process as run):

- Scripts, and scripts with OTC add-ons, follow the five steps above
  unchanged.
- An OTC-only customer stays at the drop-off counter. The dispenser captures
  the sale as normal, but the label prints to a dedicated printer in the back
  (printer 9).
- Whoever is free in the back reads that print as a signal, picks the item at
...
named measurement plan; (2) measure plus cheap reversible experiments -
formalise and instrument Irene's fast path, pull forward the digital OTC
planogram on the existing screens (both stores back it; vendor slots can
