...
- **Owns the clinical check** — DDIs, contraindications, high-alert medicines
  (insulin, anticoagulants, opioids), dose appropriateness — the step that makes
...
- Constantly **interrupted** — counter queries, phone calls, scheme rejections,
  stock-outs — fragmenting the focus the clinical check demands.
...
the prescriber on a query, and quietly catches errors the system should have
caught. She welcomes anything that removes admin — and distrusts anything that
quietly removes *her* from the loop.
