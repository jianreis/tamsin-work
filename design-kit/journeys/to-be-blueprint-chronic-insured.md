...
- **Metric:** median time-to-check; % needing pharmacist intervention (intervention is a feature of the design, a sign the safety check is working).
...
- **Fixes as-is pain:** #4 (payment friction); payment is resolved up front rather than at a failing till.
...
- **Policy/reg:** telehealth-originated scripts are **now lawful** post-COVID
  ...
  e-script needs an **ECTA AES**, so this stays **🔮 requires-reform/maturity** until the
...
SEP pricing shown transparently (it is fixed by law, so cannot be discounted) · licensed pharmacy is seller of record.
